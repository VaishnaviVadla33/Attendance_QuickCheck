...
# Creating a notebook
my_notebook = tb.Notebook(root, bootstyle="success", width=1600, height=850)
...
def update_entries_s1(event):
    ...
    if semester == "sem 5":
        ...
        total_count_t_label_s1.set(0)
        total_count_p_label_s1.set(0)
    else:
        ...
        count_t_s1.set(int(selected_row["total count"].values[0]))
        #total_count_p_label_s1.get()


    if (theory_var_s1 != 0) and (practical_var_s1 != 0):
        weeks = count_t_s1.get() / theory_var_s1.get()
    elif theory_var_s1 == 0:
        weeks = count_t_s1.get() / practical_var_s1.get()
    elif practical_var_s1 == 0:
        weeks = count_t_s1.get() / theory_var_s1.get()
    else :
        weeks = 0

    c1 = int(theory_var_s1.get() * weeks)
    c2 = int(practical_var_s1.get() * weeks)
    count_t_s1.set(c1)
    count_p_s1.set(c2)    
...
count_t_s1=tk.IntVar()
count_p_s1=tk.IntVar()

theory_label_s1 = tb.Label(s_percent_frame, text="Theory count in week:", bootstyle="success")
theory_label_s1.pack()
theory_entry_s1 = tb.Entry(s_percent_frame, textvariable=theory_var_s1, state="readonly")
...
practical_entry_s1 = tb.Entry(s_percent_frame, textvariable=practical_var_s1, state="readonly")
...
total_count_t_entry_s1 = tb.Entry(s_percent_frame,textvariable=count_t_s1, state="readonly")
...
total_count_p_entry_s1 = tb.Entry(s_percent_frame,textvariable=count_p_s1, state="readonly")
...
def calculate_estimation_m():
    ...
    try:
        ...
        if ((balance_t <= absents_to_take1) and (balance_p <= absents_to_take2)): 
            ...
            messagebox.showerror(f"RESULT", f"You are in defaulter list \n cause maximum leaves can be taken in {subject_name} theory classes is : {absents_to_take1} and \nmaximum leaves can be taken in {subject_name} practical classes is : {absents_to_take2}\n")
        elif (balance_t > absents_to_take1):
            messagebox.showerror(f"RESULT", f"You are in defaulter list, cause maximum leaves can be taken in  {subject_name} theory classes is : {absents_to_take1}")
        elif (balance_p > absents_to_take2):
            messagebox.showerror(f"RESULT", f"You are in defaulter list, cause maximum leaves can be taken in {subject_name} practical classes is : {absents_to_take2}")     
        # Display the result in a pie chart
